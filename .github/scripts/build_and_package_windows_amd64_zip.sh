...
ARCH="amd64"
TAG_NAME="${1:-local}"
BUILD_DIR="$(pwd)/build/windows/x64/runner/Release"  # مسیر اصلی خروجی اجرایی
...
package_release_zip() {
  ...
  # رفتن به دایرکتوری build
  cd "$BUILD_DIR"

  echo "📁 Zipping Windows release package..."

  # فشرده‌سازی تمام فایل‌های لازم
  zip -r "$ZIP_PATH" ./*

  echo "✅ Zip package saved to: $ZIP_PATH"

  # کپی به دایرکتوری ریشه artifacts
  cp "$ZIP_PATH" "$ROOT_OUTPUT_DIR/"
  echo "✅ Zip package copied to: $ROOT_OUTPUT_DIR"

  # نمایش اطلاعات فایل‌های ساخته شده
...
# ذخیره مسیر فایل برای مراحل بعدی
